// Dafny program nspk.dfy compiled into C#
...
// Dafny 3.0.0.30203
// Command Line Options: /Users/sword/Documents/GitHub/paraverif_dafny/security protocol/nspk.dfy /verifyAllModules /compile:1 /spillTargetCode:1 /out:bin/nspk
// nspk.dfy
...
method aencrtpy(aencMsg: message, aencKey: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  ...
  decreases aencMsg, aencKey, intruKnows
{
  var aencMsg1 := Aenc(aencMsg, aencKey);
  newIntruKnows := intruKnows + {aencMsg1};
}

method dencrtpy(msg: message, aencKey: message, aencMsg: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  ...
  decreases msg, aencKey, aencMsg, intruKnows
{
  newIntruKnows := intruKnows + {aencMsg};
}

method sencrtpy(sencMsg: message, sencKey: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  ...
  decreases sencMsg, sencKey, intruKnows
{
  var sencMsg1 := Senc(sencMsg, sencKey);
  newIntruKnows := intruKnows + {sencMsg1};
}

method dsencrtpy(msg: message, sencKey: message, sencMsg: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  requires msg in intruKnows
  requires sencKey in intruKnows
  requires sencMsg !in intruKnows
  ...
  decreases msg, sencKey, sencMsg, intruKnows
{
  newIntruKnows := intruKnows + {sencMsg};
}

method Separate(pairMsg: message, m1: message, m2: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  ...
  decreases pairMsg, m1, m2, intruKnows
{
  newIntruKnows := intruKnows + {m1, m2};
}

method Pairing(m1: message, m2: message, intruKnows: set<message>)
    returns (newIntruKnows: set<message>)
  requires m1 in intruKnows
  requires m2 in intruKnows
  requires Pair(m1, m2) !in intruKnows
  ensures Pair(m1, m2) in newIntruKnows
  decreases m1, m2, intruKnows
{
  newIntruKnows := intruKnows + {Pair(m1, m2)};
}

method ReceiveMsgFromChannel(c: channel, m1: message, intruKnows: set<message>)
    returns (m: message)
  requires c.Length > 0
  requires m1 != Nil
  requires m1 == c[0]
  ...
  ensures m1 == m
  ensures c[0] == Nil
  decreases c, m1, intruKnows
{
  c[0] := Nil;
  m := m1;
}

method SendMsgToChannel(c: channel, m: message, intruKnows: set<message>)
  requires c.Length > 0
  requires Var(""Nb"") !in intruKnows
  modifies c
  ensures Var(""Nb"") !in intruKnows
  ensures c[0] == m
  decreases c, m, intruKnows
{
  c[0] := m;
}

method AliceSendMsg_1(c: channel, intruKnows: set<message>)
  requires c.Length > 0
  requires Var(""Nb"") !in intruKnows
  modifies c
  ensures Var(""Nb"") !in intruKnows
  ensures c[0] != Nil
  decreases c, intruKnows
{
...
method AliceSendMsg_2(c: channel, intruKnows: set<message>)
  requires c.Length > 0
  requires Var(""Nb"") !in intruKnows
  modifies c
  ensures Var(""Nb"") !in intruKnows
...
method BobSendMsg_1(c: channel, intruKnows: set<message>)
  requires c.Length > 0
  requires Var(""Nb"") !in intruKnows
  modifies c
  ensures Var(""Nb"") !in intruKnows
...
method AliceGetMsg_1(c: array<message>, intruKnows: set<message>, flag: bool)
    returns (m: message)
  requires c.Length > 0
  requires flag == false
  ...
  ensures c[0] == Nil
  ensures m != Nil
  decreases c, intruKnows, flag
{
  m := c[0];
  c[0] := Nil;
}

method BobGetMsg_1(c: array<message>, intruKnows: set<message>) returns (m: message)
  requires c.Length > 0
  requires rverify(c[0])
  requires Var(""Nb"") !in intruKnows
  modifies c
...
method BobGetMsg_2(c: array<message>, intruKnows: set<message>) returns (m: message)
  requires c.Length > 0
  requires rverify(c[0])
  requires Var(""Nb"") !in intruKnows
  modifies c
...
method IntruderGetMsg(c: channel, intruKnows: set<message>, i: int)
    returns (newIntruKnows: set<message>)
  requires c.Length > 0
  requires iverify(c[0])
  modifies c
  ensures c[0] == Nil
  decreases c, intruKnows, i
{
...
method IntruderSendMsg(c: channel, m: message, intruKnws: set<message>)
  ...
  decreases c, m, intruKnws
{
  c[0] := m;
}

predicate rverify(m: message)
  decreases m
{
  match m
  case Nil() =>
    false
  case Aenc(aencMsg, aencKey) =>
    if aencKey == message.Pk(""B"") || aencKey == message.Pk(""A"") then
      true
    else
      false
  case Senc(m1, m2) =>
...
function method destruct(m: message): message
  decreases m
{
  match m
  case Nil() =>
  ...
  case Aenc(aencMsg, aencKey) =>
    if aencKey == message.Pk(""I"") then
      ...
      Aenc(aencMsg, aencKey)
  case Senc(aencMsg, sencKey) =>
    Senc(aencMsg, sencKey)
  case Var(r1) =>
    Var(r1)
  case Str(r1) =>
    Str(r1)
...
namespace _module {
  ...
  public partial class __default {
    public static Dafny.ISet<message> aencrtpy(message aencMsg, message aencKey, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      message _299_aencMsg1;
      _299_aencMsg1 = @message.create_Aenc(aencMsg, aencKey);
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(_299_aencMsg1));
      return newIntruKnows;
    }
    public static Dafny.ISet<message> dencrtpy(message msg, message aencKey, message aencMsg, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(aencMsg));
      return newIntruKnows;
    }
    public static Dafny.ISet<message> sencrtpy(message sencMsg, message sencKey, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      message _300_sencMsg1;
      _300_sencMsg1 = @message.create_Senc(sencMsg, sencKey);
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(_300_sencMsg1));
      return newIntruKnows;
    }
    public static Dafny.ISet<message> dsencrtpy(message msg, message sencKey, message sencMsg, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(sencMsg));
      return newIntruKnows;
    }
    public static Dafny.ISet<message> Separate(message pairMsg, message m1, message m2, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(m1, m2));
      return newIntruKnows;
    }
    public static Dafny.ISet<message> Pairing(message m1, message m2, Dafny.ISet<message> intruKnows)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(@message.create_Pair(m1, m2)));
      return newIntruKnows;
    }
    public static message ReceiveMsgFromChannel(message[] c, message m1, Dafny.ISet<message> intruKnows)
    {
    ...
    public static void SendMsgToChannel(message[] c, message m, Dafny.ISet<message> intruKnows)
    {
      (c)[(int)((BigInteger.Zero))] = m;
    }
    public static void AliceSendMsg__1(message[] c, Dafny.ISet<message> intruKnows)
    {
      message _301_aencMsg;
      _301_aencMsg = @message.create_Pair(@message.create_Var(Dafny.Sequence<char>.FromString("Na")), @message.create_Str(Dafny.Sequence<char>.FromString("A")));
      message _302_m;
      _302_m = @message.create_Aenc(_301_aencMsg, @message.create_Pk(Dafny.Sequence<char>.FromString("B")));
      (c)[(int)((BigInteger.Zero))] = _302_m;
    }
    public static void AliceSendMsg__2(message[] c, Dafny.ISet<message> intruKnows)
    {
      message _303_aencMsg;
      _303_aencMsg = @message.create_Var(Dafny.Sequence<char>.FromString("Nb"));
      message _304_m;
      _304_m = @message.create_Aenc(_303_aencMsg, @message.create_Pk(Dafny.Sequence<char>.FromString("B")));
      (c)[(int)((BigInteger.Zero))] = _304_m;
    }
    public static void BobSendMsg__1(message[] c, Dafny.ISet<message> intruKnows)
    {
      message _305_aencMsg;
      _305_aencMsg = @message.create_Pair(@message.create_Var(Dafny.Sequence<char>.FromString("Na")), @message.create_Var(Dafny.Sequence<char>.FromString("Nb")));
      message _306_m;
      _306_m = @message.create_Aenc(_305_aencMsg, @message.create_Pk(Dafny.Sequence<char>.FromString("A")));
      (c)[(int)((BigInteger.Zero))] = _306_m;
    }
    public static message AliceGetMsg__1(message[] c, Dafny.ISet<message> intruKnows, bool flag)
    {
      message m = message.Default();
      m = (c)[(int)(BigInteger.Zero)];
      (c)[(int)((BigInteger.Zero))] = @message.create_Nil();
      return m;
    }
    public static message BobGetMsg__1(message[] c, Dafny.ISet<message> intruKnows)
    {
      message m = message.Default();
      m = (c)[(int)(BigInteger.Zero)];
      (c)[(int)((BigInteger.Zero))] = @message.create_Nil();
    ...
    public static message BobGetMsg__2(message[] c, Dafny.ISet<message> intruKnows)
    {
      message m = message.Default();
      m = (c)[(int)(BigInteger.Zero)];
      (c)[(int)((BigInteger.Zero))] = @message.create_Nil();
    ...
    public static Dafny.ISet<message> IntruderGetMsg(message[] c, Dafny.ISet<message> intruKnows, BigInteger i)
    {
      Dafny.ISet<message> newIntruKnows = Dafny.Set<message>.Empty;
      message _307_aencMsg;
      _307_aencMsg = __default.destruct((c)[(int)(BigInteger.Zero)]);
      newIntruKnows = Dafny.Set<message>.Union(intruKnows, Dafny.Set<message>.FromElements(_307_aencMsg, (c)[(int)(BigInteger.Zero)]));
      (c)[(int)((BigInteger.Zero))] = @message.create_Nil();
      return newIntruKnows;
    }
    public static void IntruderSendMsg(message[] c, message m, Dafny.ISet<message> intruKnws)
    {
      (c)[(int)((BigInteger.Zero))] = m;
    }
    public static message destruct(message m) {
      message _source0 = m;
      if (_source0.is_Nil) {
        return @message.create_Nil();
      } else if (_source0.is_Aenc) {
        message _308___mcc_h0 = ((message_Aenc)_source0).aencMsg;
        message _309___mcc_h1 = ((message_Aenc)_source0).aencKey;
        message _310_aencKey = _309___mcc_h1;
        message _311_aencMsg = _308___mcc_h0;
        if (object.Equals(_310_aencKey, @message.create_Pk(Dafny.Sequence<char>.FromString("I")))) {
          return _311_aencMsg;
        } else {
          return @message.create_Aenc(_311_aencMsg, _310_aencKey);
        }
      } else if (_source0.is_Senc) {
        message _312___mcc_h2 = ((message_Senc)_source0).m1;
        message _313___mcc_h3 = ((message_Senc)_source0).m2;
        message _314_sencKey = _313___mcc_h3;
        message _315_aencMsg = _312___mcc_h2;
        return @message.create_Senc(_315_aencMsg, _314_sencKey);
      } else if (_source0.is_K) {
        Dafny.ISequence<char> _316___mcc_h4 = ((message_K)_source0).r1;
        Dafny.ISequence<char> _317___mcc_h5 = ((message_K)_source0).r2;
        Dafny.ISequence<char> _318_r2 = _317___mcc_h5;
        Dafny.ISequence<char> _319_r1 = _316___mcc_h4;
        return @message.create_K(_319_r1, _318_r2);
      } else if (_source0.is_Pk) {
        Dafny.ISequence<char> _320___mcc_h6 = ((message_Pk)_source0).r;
        Dafny.ISequence<char> _321_r1 = _320___mcc_h6;
        return @message.create_Pk(_321_r1);
      } else if (_source0.is_Sk) {
        Dafny.ISequence<char> _322___mcc_h7 = ((message_Sk)_source0).r;
        Dafny.ISequence<char> _323_r1 = _322___mcc_h7;
        return @message.create_Sk(_323_r1);
      } else if (_source0.is_Str) {
        Dafny.ISequence<char> _324___mcc_h8 = ((message_Str)_source0).r;
        Dafny.ISequence<char> _325_r1 = _324___mcc_h8;
        return @message.create_Str(_325_r1);
      } else if (_source0.is_Var) {
        Dafny.ISequence<char> _326___mcc_h9 = ((message_Var)_source0).n;
        Dafny.ISequence<char> _327_r1 = _326___mcc_h9;
        return @message.create_Var(_327_r1);
      } else {
        message _328___mcc_h10 = ((message_Pair)_source0).m1;
        message _329___mcc_h11 = ((message_Pair)_source0).m2;
        message _330_m2 = _329___mcc_h11;
        message _331_m1 = _328___mcc_h10;
        return @message.create_Pair(_331_m1, _330_m2);
      }
    }
  }
