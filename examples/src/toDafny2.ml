...
let gen_var_str trees =
  ...
  let rec wrapper tree =
    match tree with
    | Node(name, pds, [], tname) -> let t = Hashtbl.replace vartable ~key: name ~data:(if List.is_empty pds then (sprintf "array<%s>" (get_arr_of pds tname name)) else (get_arr_of pds tname name)) in sprintf "%s : %s" name (get_arr_of pds tname name);
    | Node(name, pds, nodes, tname) ->
      begin
        ...
        | Some(record_name) ->
          ...
          (* (List.map ~f:(fun t-> if contains t x then Hashtbl.replace vartable ~key:(name^"_"^t ~data:Hashtbl.find_exn second t)) (Hashtbl.keys secondtable)) *)
          String.concat ~sep:",\n" (
...
let rule_act  types  nameAdd extraInvParas reqStr ensureStr varNamesOfInv  r=
  ...
  if pds = [] then
    ...
    let paramStr="()" in
    sprintf "\nmethod %s%s\n%s\n%s\n%s{\n%s\n}" name 
    (sprintf "(%s\n%s\n%s)" pds_str (param_str1^"N0:nat,") (if (extraInvParas="") then ("p__Inv0:nat,p__Inv2:nat") else (extraInvParas)) )
 (reqStr^guard) 
    ...
    let rstr =
      ...
      (name^nameAdd)
      (sprintf "(%s\n%s,\n%s)" pds_str (param_str1^",N0:nat") (if (extraInvParas="") then ("p__Inv0:nat,p__Inv2:nat") else (extraInvParas)) )
      (sprintf "\n%s\n%s\n%s%s\n%s\n" param_constr_str2 new_constr param_str (if (extraInvParas="") then ("") else (reqStr)) guard)
      (ensureStr)  
      allModifyConstr 
       sp statements sp in
       rstr
