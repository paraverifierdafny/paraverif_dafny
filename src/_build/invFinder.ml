...
let minify_inv_desc inv =
  ...
  let s1=String.Set.union_list (List.map ~f:(fun x-> String.Set.of_list [get_rname_of_crname (ToStr.Smv.form_act ~lower:false x)]) ls) in 
  let temp2 =List.map ~f:(fun x->print_endline ("Part2:"^( x))) (String.Set.to_list s1) in 
  let ls1 =List.map ~f:(fun x->(List.filter ls ~f:(fun y->List.exists ls (get_rname_of_crname (ToStr.Smv.form_act ~lower:false y)=x)))) temp2  in
