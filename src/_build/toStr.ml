...
module Smv = struct
  ...
  *)
  and form_act ?(lower=true) form =
    ...
    | ForallFormula(pds,f) -> sprintf "(forall %s::%s)" (String.concat ~sep:"," (List.map pds ~f:(fun x-> let Paramdef(vn,tn)=x in sprintf "%s" vn ))) (form_act ~lower f)
    | ExistFormula(pds,f) -> sprintf "(exists %s::%s)" (String.concat ~sep:"," (List.map pds ~f:(fun x-> let Paramdef(vn,tn)=x in sprintf "%s" vn ))) (form_act ~lower f)
